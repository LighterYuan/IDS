...
def parse_args():
    parser = argparse.ArgumentParser(description='捕获攻击网络流量')
    parser.add_argument('-i', '--interface', required=True, help='网络接口名称')
    parser.add_argument('-c', '--count', type=int, default=100000, help='捕获的数据包数量（默认：100,000）')
    parser.add_argument('-o', '--output', default='../data/attack/', help='输出目录')
    parser.add_argument('-t', '--attack-type', required=True, help='攻击类型（如：dos, scan, etc.）')
    
    return parser.parse_args()

def capture_traffic(interface, count, output_dir, attack_type):
    """使用tshark捕获网络流量"""
    ...
    output_file = os.path.join(output_dir, f"{attack_type}_attack_{timestamp}.pcap")
    
    print(f"开始捕获{attack_type}攻击流量，接口：{interface}，目标数据包数：{count}")
    ...
    try:
        ...
        if process.returncode == 0:
            print(f"成功捕获{attack_type}攻击流量，已保存至: {output_file}")
...
def main():
    ...
    # 捕获流量
    pcap_file = capture_traffic(args.interface, args.count, args.output, args.attack_type)
